...
## Nikon D3500 shutter speeds
ss = np.array([1/1000, 1/500, 1/250, 1/125, 1/60, 1/30, 1/15, 1/8, 1/4, 1/2, 1])
...
def mergeOptimal(images, type, weight, g=None) :
    t_nc = 1 / 1.6
    darkFrame = np.load('darkFrame.npy')
    for i in range(images.shape[0]) :
        images[i] = images[i] - darkFrame * (ss[i] / t_nc)
    images_ldr = np.array(images, dtype=np.float32) / (pow(2, 16) - 1)
...
## Optimal weights merging
images = None
for i in range(1, 12) :
    filename = '../data/campus/campus' + str(i) + '.tiff'
    image = skimage.io.imread(filename)
    if i == 1 :
        images = np.zeros((11, image.shape[0], image.shape[1], image.shape[2]), dtype=np.int32)
    images[i - 1] = image
image_hdr = mergeOptimal(images, 'tiff', w_optimal, None)
filename = 'campus_log_optimal' + '.HDR'
...
weightFuncs = [w_uniform, w_tent, w_gaussian, w_photon]
...
for x in typeSelect :
    ...
    images = None
    for i in range(1, 12) :
        filename = '../data/campus/campus' + str(i) + '.' + type
        image = skimage.io.imread(filename)
        if i == 1 :
            images = np.zeros((11, image.shape[0], image.shape[1], image.shape[2]), dtype=np.int32)
        images[i - 1] = image
    ...
    for y in weightSelect :
        ...
        for z in mergeAlgSelect:
            ...
            filename = 'campus' + name1 + name2 + name3 + '.HDR'
            writeHDR(filename, image_hdr)
            print('Written to ' + filename)
